...
var server = app.listen(port, function () {
  console.log(
    "node.js static server listening on port: " +
      port +
      ", with websockets listener"
  );
});
...
function cleanUp() {
  ...
  for (var i = 0; i < eightDecks.length; i++) {
    ...
    if (eightSockets.length > 0) {
      if (
        eightSockets[playspot] == null ||
        eightSockets[playspot + 1] == null
      ) {
  ...
  if (eightDecks.length == 0) {
    //no game essentials left
    let deck = new Deck();
    deck.shuffle();
...
function sendMessage(message) {
  //For a later use of chatroom function
...
function eightsPlay(ws) {
  ...
  if (index % 2 == 0) {
    ...
    obj.numberOfOpponentCards =
      crazyEightPlayers[index + 1].getHandCopy().length;
...
function cardSelected(message, playerNumber) {
  ...
  if (crazyEightPlayers[playerNumber].isHandEmpty()) {
    ...
    obj.numberOfOpponentCards =
      crazyEightPlayers[otherplayerNumber].getHandCopy().length;
    obj.pileTopCard = eightPiles[gamenum].getTopCard();
    obj.pileAnnouncedSuit = eightPiles[gamenum].getAnnouncedSuit();
    ...
    obj.numberOfOpponentCards =
      crazyEightPlayers[playerNumber].getHandCopy().length;
    obj.pileTopCard = eightPiles[gamenum].getTopCard();
    obj.pileAnnouncedSuit = eightPiles[gamenum].getAnnouncedSuit();
    ...
    obj.numberOfOpponentCards =
      crazyEightPlayers[playerNumber - 1].getHandCopy().length;
    obj.pileTopCard = eightPiles[gamenum].getTopCard();
    obj.pileAnnouncedSuit = eightPiles[gamenum].getAnnouncedSuit();
    obj.yourCards = crazyEightPlayers[playerNumber].getHandCopy();
    obj.readyToPlay = false;
...
function cardPicked(playerNumber) {
  ...
  obj.status =
    "You selected card " + drawnCard.getValue() + drawnCard.getSuit();
  obj.numberOfOpponentCards =
    crazyEightPlayers[otherplayerNumber].getHandCopy().length;
  obj.pileTopCard = eightPiles[gamenum].getTopCard();
  obj.pileAnnouncedSuit = eightPiles[gamenum].getAnnouncedSuit();
...
function snipPlay(ws) {
  ...
  if (snipSockets.indexOf(ws) % 2 == 0) {
    ...
    obj.snap = false;
    obj.numberOfOpponentCards =
      snipPlayers[snipSockets.indexOf(ws) + 1].getHandCopy().length;
...
function victory(message, playerNumber) {
  ...
  let newHand = JSON.parse(JSON.stringify(hand), (k, v) =>
    typeof v.suit !== "undefined" ? new Card(v.suit, v.value) : v
  );
  ...
  obj.snap = false;
  obj.numberOfOpponentCards =
    snipPlayers[otherplayerNumber].getHandCopy().length;
  obj.readyToPlay = false;
...
function snipQuit(playernumber) {
  ...
  if (playernumber % 2 == 0) {
    ...
    if (snipSockets[playernumber + 1] != null && !snipComplete[gamenum]) {
      ...
      obj.status = "You win!! Opponent has left the game!";
      obj.numberOfOpponentCards =
        snipPlayers[playernumber].getHandCopy().length;
      obj.snip = false;
      obj.snap = false;
...
function goFish(message, playerNumber) {
  //Go Fish player has said go fish
  let empty = false;
  let score = false;
  let otherplayerNumber = 0;
  ...
  console.log(
    "This many cards left in the deck" + fishDecks[gamenum].list.length
  );

  let c = null;
  let obj = {};
  if (fishDecks[gamenum].list.length == 0) {
    //No cards left to draw from the pile
    ...
    obj.numberOfOpponentCards =
      fishPlayers[otherplayerNumber].getHandCopy().length;
    obj.askCard = null;
    obj.fish = true;
    obj.readyToPlay = true;
    ...
    setTimeout(() => {
      ...
      if (c != null) {
        ...
        console.log(
          playerNumber + " Player num four ofs " + fourOfs[playerNumber - 1]
        );
        //check if the game is over
        if (
          fishDeck.list.length == 0 &&
          fishPlayers[playerNumber].list.length == 0 &&
          fishPlayers[otherplayerNumber].list.length == 0
        ) {
          //Check for a winner
          ...
          if (phand.length == 0) {
            //if so that player draws cards from the deck
            ...
            console.log(
              "This many cards left in the deck" + fishDeck.list.length
            );
          ...
          obj.yourCards = fishPlayers[otherplayerNumber].getHandCopy();
          obj.numberOfOpponentCards =
            fishPlayers[playerNumber].getHandCopy().length;
          obj.fish = false;
          obj.score = c;
          obj.askCard = null;
          obj.readyToPlay = false;
...
function giveCard(message, playerNumber) {
  ...
  let tempcards = JSON.parse(JSON.stringify(giveCards), (k, v) =>
    typeof v.suit !== "undefined" ? new Card(v.suit, v.value) : v
  );
  let hand = message.hand;
  let newHand = JSON.parse(JSON.stringify(hand), (k, v) =>
  ...
  setTimeout(() => {
    ...
    if (c != null) {
      ...
      if (
        fishDecks[gamenum].list.length == 0 &&
        fishPlayers[playerNumber].list.length == 0 &&
        fishPlayers[otherplayerNumber].list.length == 0
      ) {
        fishWinner(message, playerNumber);
...
function checkAmount(player) {
  ...
  for (var i = 0; i < newHand.length; i++) {
    ...
    if (total == 4) {
      ...
      while (true) {
        let hand = player.getHandCopy();
        let Hands = JSON.parse(JSON.stringify(hand), (k, v) =>
          typeof v.suit !== "undefined" ? new Card(v.suit, v.value) : v
        );
...
function countCard(card, player) {
...
function quitFish(playernumber) {
  ...
  if (playernumber % 2 == 0) {
    ...
    if (fishSockets[playernumber + 1] != null && !fishComplete[gamenum]) {
      ...
      obj.status = "You win!! Opponent has left the game!";
      obj.numberOfOpponentCards =
        fishPlayers[playernumber + 1].getHandCopy().length;
      obj.askCard = null;
      obj.yourCards = fishPlayers[playernumber].getHandCopy();
      obj.readyToPlay = false;
